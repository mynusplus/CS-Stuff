...
def frame_images(original_image, color, wide):
    width, height = original_image.size
    img2 = original_image.crop((wide, wide, width-wide, height-wide))
    background = PIL.Image.new('RGBA', (width, height), color)
    result = background
    result.paste(img2, (wide, wide), mask = None)
    return result
    
def mirror_images(original_image):
    width, height = original_image.size
    img2 = original_image.crop((0, 0, width/2, height))
    rotated_image = img2.transpose(PIL.Image.FLIP_LEFT_RIGHT)
    result = original_image
    result.paste(rotated_image, (int(width/2), 0), mask = None)
    return result
...
def frame_all_images(directory=None):
    ...
    Uses current directory if no directory is specified. 
    Places images in subdirectory 'modified', creating it if it does not exist.
    New image files are of type PNG and have frame.
    """
    ...
    for n in range(len(image_list)):
        ...
        # Round the corners with radius = 30% of short side
        new_image = frame_images(image_list[n],(0, 255, 255), 10)
        #save the altered image, suing PNG to retain transparency
        new_image_filename = os.path.join(new_directory, filename + '.png')
        new_image.save(new_image_filename) 
...
def mirror_all_images(directory=None):
    """ Saves a modfied version of each image in directory.
    
    Uses current directory if no directory is specified. 
    Places images in subdirectory 'modified', creating it if it does not exist.
    New image files are of type PNG and have been mirrored across the middle.
    """
    ...
    for n in range(len(image_list)):
        ...
        # Round the corners with radius = 30% of short side
        new_image = mirror_images(image_list[n])
        #save the altered image, suing PNG to retain transparency
        new_image_filename = os.path.join(new_directory, filename + '.png')
        new_image.save(new_image_filename) 