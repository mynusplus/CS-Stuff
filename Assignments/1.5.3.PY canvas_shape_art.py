...
######
# Instantiate two radio buttons
#####  
tkinter.Radiobutton(root, text="Circles", variable=shape_intvar, value=0).grid(row=3, column=1)
tkinter.Radiobutton(root, text="Rectangles", variable=shape_intvar, value=1).grid(row=3, column=2)

######
# Inform user of mouse interface
######
message = tkinter.Label(root, text='Drag mouse to\ndraw circles.\nDrag sliders\nto change color.')
message.grid(column=0, row=0,  rowspan = 3, sticky=tkinter.N)

######
# Inform user of button interface
######
message = tkinter.Label(root, text='Select which shape to draw with.', anchor = 'center')
message.grid(column=1, row=3, sticky=tkinter.N)
...
def up(event):
    ...
    if shape_intvar.get() == 0: #Shape is circle
    ...
    elif shape_intvar.get() == 1: #Shape is square
